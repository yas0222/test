...
6. SHA 登録後にダウンロードし直した最新の `google-services.json` を **`android/app/google-services.json`** に上書き配置する。
7. 反映後に次を実行する。
...
npx cap sync android
```
8. `android/build.gradle`（または `build.gradle.kts`）で Google Services Gradle Plugin（`com.google.gms.google-services`）を有効化し、`android/app/build.gradle` 側で apply されていることを確認する（Capacitor/Firebase プラグイン導入時に自動反映される構成が多い）。
...
- `capacitor.config.ts` の `plugins.FirebaseAuthentication` で `skipNativeAuth: false` と `providers: ["google.com"]` を設定し、`GoogleAuthProviderHandler` が初期化される前提条件を満たす。
